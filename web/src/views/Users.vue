<template>
  <div class="card">
    <div class="card-title">
      <h2>{{ $t('mangerUser.users') }}</h2>
      <router-link to="/users/new"><button class="button">{{ $t('buttons.new') }}</button></router-link>
    </div>

    <div class="card-content full">
      <table>
        <tr>
          <th>{{ $t('mangerUser.username') }}</th>
          <th>{{ $t('mangerUser.admin') }}</th>
          <th>{{ $t('mangerUser.scope') }}</th>
          <th></th>
        </tr>
